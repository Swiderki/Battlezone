...
class Enemy extends PhysicalGameObject {
    ...
    randomMove() {
        ...
        if (Math.random() < this.moveTowardsPlayerChance) {
            ...
            const destination = Vector.add(this.game.player.position, Vector.multiply(normalizedDirection, keepDistanceFromPlayer));
        ...
        for (const obstacle of this.game.obstacles) {
            if (collideObjects([Vector.add(this.boxCollider![0], destination), Vector.add(this.boxCollider![1], destination)], obstacle.boxCollider!)) {
    ...
    rotateTowards(destiny: Vec3D) {
        ...
        // normalize to [-π, π]
        angleDifference = ((angleDifference + Math.PI) % (2 * Math.PI) + 2 * Math.PI) % (2 * Math.PI) - Math.PI;
